// Lucide-style inline SVG icons (ISC-licensed path data), rendered without a runtime dependency.
...
const PATHS = {
  ...
  restore: '<rect x="3" y="7" width="14" height="14" rx="1.5"/><path d="M7 7V4.5A1.5 1.5 0 0 1 8.5 3h11A1.5 1.5 0 0 1 21 4.5v11a1.5 1.5 0 0 1-1.5 1.5H17"/>',
...
export function renderIcons(root = document) {
  root.querySelectorAll('i[data-icon]').forEach((el) => {
    ...
    wrapper.innerHTML = iconSvg(el.dataset.icon, size);
    const svg = wrapper.firstElementChild;
    if (el.id) svg.id = el.id;
    if (el.className) svg.classList.add(...el.classList);
    el.replaceWith(svg);
  });
}

export function setIcon(svgEl, name) {
  if (!svgEl) return;
  svgEl.innerHTML = PATHS[name] || PATHS.info;
}
